...
const App: React.FC = () =>
{
  ...
  async function test ()
  {
    ...
    if (col) {
      colors.forEach(({ name, val }) => { wf.variable.set.color({ name, val, col }) })
      sizeG.forEach(({ name, val }) => { wf.variable.set.size({ name, val, col }) });
      sizeT.forEach(({ name, val }) => { wf.variable.set.size({ name, val, col }) });
