...
// ----- types -----
type Fallback = 'if_exist_replace' | 'if_exist_return'

type VariableGenerate =
    & { set: 'byName'; name: string; fallback: Fallback }
    & (
...
async function upsertByName (
    ...
    gen: VariableGenerate
): Promise<Variable>
{
    const existing = await collection.getVariableByName(gen.name)
    if (existing) {
        if (gen.fallback === 'if_exist_return') return existing
        await existing.set(gen.value, { mode: 'replace' })
        return existing
    }
    switch (gen.type) {
        case 'number': return collection.createNumberVariable(gen.name, gen.value)
        case 'color': return collection.createColorVariable(gen.name, gen.value)
        case 'font': return collection.createFontFamilyVariable(gen.name, gen.value)
        case 'size': return collection.createSizeVariable(gen.name, gen.value)
    }
}
